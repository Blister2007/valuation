...
> A fully local, formula-driven equity valuation tool. No API. No backend. No cost.
...
Open an annual report, enter the numbers, get a full institutional-grade equity valuation — entirely in your browser. Every calculation runs locally. Nothing is sent anywhere.

Built around the **FCFF (Free Cash Flow to Firm)** methodology used in CFA-level equity research, with WACC as the discount rate and an EV→Equity bridge to arrive at intrinsic value per share.
...
| **DuPont Decomposition** | Net Margin × Asset Turnover × Equity Multiplier → ROE verification |
...
# Open in browser
open valuation_v3.html
```

Or just [download the HTML file](./valuation_v3.html) and open it directly. That's it.
...
3. **Enter actual currency units** (not thousands or millions) — the tool formats them automatically
4. Click **CALCULATE FULL VALUATION →**
5. Navigate across tabs: **DCF MODEL → DUPONT → SENSITIVITY → VERDICT**
...
| Risk-Free Rate | 10-year government bond yield (India ≈ 7%, US ≈ 4.5%) |
...
Growth fades linearly from sustainable rate → terminal rate over the 5-year projection window.
...
The tool uses built-in sector averages for multiple comparisons:
...
- **Zero dependencies** — pure HTML, CSS, and vanilla JS. The chart is drawn on an HTML5 `<canvas>`, no Chart.js or D3.
- **All computation is local** — no data leaves the browser at any point.
- **Print / PDF ready** — use the "Print / Save as PDF" button on the Verdict tab for a clean single-page export.
- **Input validation** — alerts fire if WACC ≤ 0 or if terminal growth rate ≥ WACC (both would break the Gordon Growth Model).
...
- The FCFF growth projection uses a linear fade from ROIC-based sustainable growth to terminal. This is a simplification; it doesn't model segment-level growth or step-changes.
